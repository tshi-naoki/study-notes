转载：http://hollischuang.gitee.io/tobetopjavaer/#/basics/java-basic/syntactic-sugar
...
首先，我们发现，在反编译后的代码中没有System.out.println("Hello, ONLINE!");，这其实就是条件编译。当if(ONLINE)为false的时候，编译器就没有对其内的代码进行编译。在编译的时候就预知道判定语句的结果，如果为false，就不会编译。
